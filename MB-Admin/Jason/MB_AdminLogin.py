...
from MB_AdminLocators import MB_Admin_Login_Objects
from MB_AdminLocators import MB_Admin_Home_Objects
...
class MB_AdminLogin():
    ...
    def Launch_Login_Page(self):
        self.driver.get("http://megabytes-admin-frontend.s3-website-us-east-1.amazonaws.com/login")


    def login(self, reporter, ssPath, username, password):
        print("***Logging into MegaBytes Admin's Page***")
        driver = self.driver
        #wait for page to load
        WebDriverWait(driver, 60).until(EC.element_to_be_clickable((MB_Admin_Login_Objects.By_username_field)))
        #set username field
        userFieldObj = driver.find_element(*MB_Admin_Login_Objects.By_username_field)
        ...
        if userFieldObj.get_attribute('value') == username:
            ...
            return
        #set password field
        passwordFieldObj = driver.find_element(*MB_Admin_Login_Objects.By_password_field)
        ...
        signinObj = driver.find_element(*MB_Admin_Login_Objects.By_sign_in)
        signinObj.click()

        print("Clicking sign in button")
        #wait for login to load
        try:
            WebDriverWait(driver, 90).until(EC.url_contains("http://megabytes-admin-frontend.s3-website-us-east-1.amazonaws.com/user"))
    ...
    def logout(self, reporter, ssPath):
        ...
        if len(driver.find_elements(*MB_Admin_Home_Objects.By_signout))>0:
            try:
                WebDriverWait(driver, 5).until(EC.element_to_be_clickable((MB_Admin_Home_Objects.By_signout)))
                driver.execute_script("arguments[0].scrollIntoView();", driver.find_element(*MB_Admin_Home_Objects.By_signout))
                time.sleep(1)
                located = True
        ...
        if located:
            ...
            WebDriverWait(driver, 60).until(EC.element_to_be_clickable((MB_Admin_Home_Objects.By_signout)))
            #time.sleep(1)
            try:
                driver.find_element(*MB_Admin_Home_Objects.By_signout).click()
                WebDriverWait(driver, 30).until(EC.element_to_be_clickable((MB_Admin_Login_Objects.By_username_field)))
            except:
                pass
            #check for login page objects to confirm signout success
            if len(driver.find_elements(*MB_Admin_Login_Objects.By_username_field))>0:
                ...
                driver.get("http://megabytes-admin-frontend.s3-website-us-east-1.amazonaws.com/login")
        else:
    ...
    def bad_login(self, reporter, ssPath, username, password):
        ...
        print("***Logging into MegaBytes Admin's Page - Negative Testing***")
        ...
        if username=="" or password=="":
            time.sleep(1)
            try:  
                WebDriverWait(driver, 60).until(EC.text_to_be_present_in_element((MB_Admin_Login_Objects.By_sign_in_error), "Please enter credentials."))
            ...
            try:
                WebDriverWait(driver, 60).until(EC.text_to_be_present_in_element((MB_Admin_Login_Objects.By_sign_in_error), "Invalid Credentials"))
                reporter.reportStep("Press submit and login","A login error should appear","Login unsuccessful and an error appeared",True,"", driver.find_element(By.TAG_NAME, "body").screenshot, ssPath + ''.join(random.choices(string.ascii_lowercase, k=20)))
